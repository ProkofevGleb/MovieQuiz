...
class QuestionFactory: QuestionFactoryProtocol {
    ...
    func requestNextQuestion() {
        DispatchQueue.global().async { [self] in
            ...
            let options = ["больше", "меньше"]
            let comparing = options.randomElement()!
            
            let randomNumber = Int.random(in: 8...9)
            
            let text = "Рейтинг этого фильма \(comparing) чем \(randomNumber)?"
